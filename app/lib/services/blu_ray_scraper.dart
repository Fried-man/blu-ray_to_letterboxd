...
class BluRayScraper {
  ...
  Future<List<BluRayItem>> _fetchPrinterFriendlyData(String userId) async {
    logger.logScraper('Fetching collection data for user: $userId (Web: ${kIsWeb})');

    try {
      // First, get the main collection page to identify all categories
      final categories = await _fetchCollectionCategories(userId);
      logger.logScraper('Found ${categories.length} categories: ${categories.join(', ')}');
      ...
      for (final categoryId in categories) {
        try {
          ...
          logger.logScraper('Added ${categoryItems.length} items from category $categoryId');
        } catch (e) {
          logger.logScraper('Failed to fetch category $categoryId', error: e);
          // Continue with other categories even if one fails
        }
      }

      logger.logScraper('Successfully fetched ${allItems.length} total items from all categories');
      return allItems;
    } catch (e) {
      logger.logScraper('Failed to fetch collection data', error: e);
      rethrow;
    }
  }
  ...
  Future<List<String>> _fetchCollectionCategories(String userId) async {
    ...
    for (final match in matches) {
      if (match.groupCount >= 1) {
        ...
        if (categoryId != null && !categories.contains(categoryId)) {
          ...
          logger.logScraper('Found category ID: $categoryId');
        }
      }
    }
    ...
    for (final match in linkMatches) {
      if (match.groupCount >= 2) {
        ...
        if (categoryId != null && !categories.contains(categoryId)) {
          categories.add(categoryId);
          logger.logScraper('Found category link: $categoryId - $categoryName');
        }
      }
    }
    ...
    if (categories.isEmpty) {
      ...
      logger.logScraper('Using priority category IDs (including user-mentioned 7): ${priorityCategories.join(', ')}');
    }

    // Also include empty category for main collection
    categories.insert(0, '');

    logger.logScraper('Total categories to process: ${categories.length}');
  ...
  Future<List<BluRayItem>> _fetchCategoryPage(String userId, String categoryId) async {
    ...
    final url = kIsWeb
        ? '/collection/$userId?action=printerfriendly&showcovers=1$categoryParam'
        : '/community/collection.php?$baseParams$categoryParam';

    logger.logScraper('Fetching category $categoryId: $url');
